...
class AddNewRowsandColumns:
    ...
    def NewrowsAndcolumns(self):
        ...
        if os.path.exists(self.file_name):
            ...
            # Add header
            self.worksheet.cell(row=1, column=self.column_index, value="Reviews")
            
            # Add data to all rows
            for row in range(2, self.worksheet.max_row + 1):
                self.worksheet.cell(row=row, column=self.column_index, value="No reviews yet")
